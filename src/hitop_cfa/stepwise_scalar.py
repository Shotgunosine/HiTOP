...
# ID.fac = "std.lv"). assert_level_adds_df runs before every permuteMeasEq
# delta test.
#
# Scalar mechanics under W&E (author decision, 2026-08-18): the scalar
# model imposes intercept invariance by FIXING group-2 intercepts back to 0
# (and strict fixes group-2 residual variances back to 1) -- there are no
# "==" equality constraints for permuteMeasEq's MI machinery, so the
# scalar delta test runs as the param-free OMNIBUS permutation, and the
# per-item removal signal comes from lavaan::modindices on the scalar fit
# (score test for freeing each fixed group-2 intercept).
#
...
def cfa_test_scalar_with_mi(scalename, list_of_items, mydata_python, mydata_temp_path,
    ...
    thresholds_item_mis holds the THRESHOLD-equality MIs (op == "|") and
    metric_item_mis the LOADING MIs (op == "=~"), both from permuteMeasEq's
    @MI.obs; scalar_item_mis holds the INTERCEPT MIs from
    lavaan::modindices on the scalar fit (score test for freeing each
    fixed group-2 intercept -- see extract_item_mis_from_scalar). Each is
    populated only when its level failed.
    """
    set_seeds(12345)
    ...
    # them, so param="intercepts" has no constraints to point at (see
    # module header)
...
# -----------------------------------------------------------------------------
# Extract per-item INTERCEPT MIs from the scalar model via lavaan::modindices.
# Under Wu-Estabrook the scalar level fixes the non-reference group's
# intercepts back to 0 (no "==" equality constraints exist), so
# permuteMeasEq's @MI.obs has nothing for intercepts. modindices() supplies
# the equivalent signal: the score test for freeing each fixed group-2
# intercept, i.e. "which item's intercept most differs across groups".
# Unadjusted (no permutation Tukey correction) -- used only to RANK items
# for removal, consistent with how MIs are used at the other levels. The
# unscaled `mi` column is used, matching the X2 convention of the
# threshold/loading extractors.
# -----------------------------------------------------------------------------
def extract_item_mis_from_scalar(item_list, r_fit_name="fit_scalar"):
    """Per-item intercept MIs from lavaan::modindices on the fitted scalar
    model stored in the R global ``r_fit_name``.

    Returns dict[item -> max MI across non-reference groups], or None.
    """
    try:
        ro.r(f'''
            mi_all <- lavaan::modindices({r_fit_name})
            mi_intercepts <- mi_all[mi_all$op == "~1" & mi_all$group > 1, ]
        ''')
        ...
        if mi_df is None or len(mi_df) == 0:
            return None
        mi_df = mi_df[mi_df['lhs'].isin(item_list)]
        if len(mi_df) == 0:
            return None

        item_mis = mi_df.groupby('lhs')['mi'].max().to_dict()
