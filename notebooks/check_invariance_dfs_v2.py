...
With --measeq, fits the Wu & Estabrook (2016) models generated by
semTools::measEq.syntax instead of plain cfa(group.equal=...), adding a
"thresholds" level. Use this to validate the measEq fix: every level
should then add df.
...
def fit_level_measeq(items, group_equal, parameterization="theta"):
    """Fit one level using semTools::measEq.syntax (Wu & Estabrook 2016)."""
...
def check_scale(scale, data, temp_path, parameterization="theta",
                verbose_params=False, use_measeq=False):
    ...
    mode = 'measEq.syntax / Wu-Estabrook' if use_measeq else 'plain cfa(group.equal=)'
    print(f"\n{'='*64}\n{scale}  ({k} items, 4 response categories assumed) [{mode}]\n{'='*64}")
    print(f"{'level':<12}{'df':>8}{'npar':>8}{'chisq':>12}{'d_df':>8}{'d_npar':>8}")

    prev_df = prev_npar = None
    dfs = {}
    level_list = MEASEQ_LEVELS if use_measeq else LEVELS
    for level, group_equal in level_list:
        try:
            if use_measeq:
                df, npar, chisq = fit_level_measeq(items, group_equal,
                                                   parameterization)
            else:
    ...
    verdicts = []
    if use_measeq:
        names = [lv for lv, _ in MEASEQ_LEVELS if lv in dfs]
        for lo, hi in zip(names, names[1:]):
            d = dfs[hi] - dfs[lo]
            if d <= 0:
                verdicts.append(f"{hi.upper()} VACUOUS vs {lo} (d_df={d:+.0f})")
        if not verdicts:
            verdicts.append("All measEq levels add df; comparisons are real.")
        for v in verdicts:
            print(f"  >> {v}")
        return dfs
...
def main():
    ...
    ap.add_argument('--parameterization', default='theta',
                    choices=['theta', 'delta'])
    ap.add_argument('--measeq', action='store_true',
                    help='fit Wu & Estabrook (2016) models via '
                         'semTools::measEq.syntax (adds a thresholds level)')
    ...
    for scale in scales:
        dfs = check_scale(scale, data, args.temp,
                          ...
                          use_measeq=args.measeq)
        if dfs is not None:
            summary[scale] = dfs

    if len(summary) > 1 and not args.measeq:
        print(f"\n{'='*64}\nSUMMARY (df by level)\n{'='*64}")
